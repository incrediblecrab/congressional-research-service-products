...
[`pipeline.yml`](pipeline.yml) is the only automation. It runs on its schedule, or by hand from the Actions tab.
...
- `sync`, scheduled every 5 minutes and daily: `probe` makes one API request and reads the manifest. Only when it says a sync is needed does `run` sync within its budget, then `verify --live` and `squash` if it committed. A dispatch with `args` runs a bounded test instead, with no probe, verify, squash or `continue`.
